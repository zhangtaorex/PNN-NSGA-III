...
class res_connection(nn.Module):
    def __init__(self,input_dim):
        super(res_connection,self).__init__()
        self.layer1 = nn.Sequential(nn.Linear(input_dim, 2*input_dim), nn.SELU())
        self.layer2 = nn.Sequential(nn.Linear(2*input_dim, input_dim))
...
class parallel_net(nn.Module):
    ...
    def __init__(self, model_dim, hidden_size_skin, hidden_size_stringer, hidden_size_features) -> None:
        ...
        self.fc0 = nn.Sequential(nn.Linear(12,8), nn.SELU(), nn.Linear(8,4),nn.SELU(),nn.Linear(4,2),nn.SELU())
        self.fc1 = nn.Sequential(nn.Linear(4,8),nn.SELU(),nn.Linear(8,16),nn.SELU(),nn.Linear(16,32),nn.SELU(),nn.Linear(32,64),nn.SELU(),
                                 nn.Linear(64,128),nn.SELU(),nn.Linear(128,256),nn.SELU(),
                                 nn.Linear(256,512),nn.SELU(),nn.Linear(512, hidden_size_features),nn.SELU())
        # self.fc1 = nn.Sequential(nn.Linear(14,16),nn.SELU(),nn.Linear(16,32),nn.SELU(),nn.Linear(32,64), nn.SELU(),
        #                          nn.Linear(64,128),nn.SELU(),nn.Linear(128,256),nn.SELU(),
        #                          nn.Linear(256,512),nn.SELU(),nn.Linear(512, hidden_size_features),nn.SELU())
        
        self.fc2 = nn.Sequential(
                                 nn.Linear(6*hidden_size_skin+6*hidden_size_stringer+hidden_size_features, 4096),nn.SELU(),
                                 nn.Linear(4096,2048),nn.SELU(),nn.Linear(2048,1024),nn.SELU(),nn.Linear(1024,512),nn.SELU(),
                                 nn.Linear(512,256),nn.SELU(),nn.Linear(256,128), nn.SELU(),
                                 nn.Linear(128,64),nn.SELU(),nn.Linear(64,32),nn.SELU(),nn.Linear(32,16),nn.SELU(),
                                 nn.Linear(16,8),nn.SELU(),nn.Linear(8,4),nn.SELU(),
                                 nn.Linear(4,1)
                                 )
    ...
    def forward(self,skin_features, stringer_features, other_features):
        ...
        other_feas1 = self.fc0(other_features[:,0:12])
        other_feas = torch.cat([other_feas1, other_features[:,12:]], dim=1)
        other_feas = self.fc1(other_feas)
        # other_feas = self.fc1(other_features)
