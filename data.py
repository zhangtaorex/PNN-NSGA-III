...
def get_loader(batch_size):
    ...
    jianqie_buckling_load = pd.read_csv("E://second paper//buckling_load//jianqiebuckling_load.csv", header=None)
    # buckleload = torch.tensor(buckleload.values[:3000], dtype=torch.float32)
    skin_layerup_list, striger_layerup_list,otherfeatures = get_layup(6000)
    index = jianqie_buckling_load[jianqie_buckling_load[0].str.endswith("job")== True].index
    otherfeatures = otherfeatures[jianqie_buckling_load[0].str.endswith("job")!= True]
    skin_layerup_list = [x for i, x in enumerate(skin_layerup_list) if i not in index]
    striger_layerup_list = [x for i, x in enumerate(striger_layerup_list) if i not in index]
    buckleload = buckleload.drop(index)
